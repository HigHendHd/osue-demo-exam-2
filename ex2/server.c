...
void task_1a(void)
{
    ...
    if (ftruncate(shmfd, sizeof(struct shm_struct)) == -1) {
        error_exit("Could not truncate file");
    }

    shmp = mmap(NULL, sizeof(struct shm_struct), PROT_READ | PROT_WRITE, MAP_SHARED, shmfd, 0);
    if (shmp == MAP_FAILED) {
...
void task_2(void)
{
    while (!quit) {
        ...
        if (sem_wait(sem_server) == -1) {
            if (errno == EINTR) {
                fprintf(stdout, "Waiting interruped by signal.\n");
                break;
            }
            error_exit("Error while waiting.\n");
        }
        task_3(shmp);
...
void task_3(shm_data_t * shmp)
{
    ...
    for (int i = 0; i < num_bank_accounts; i++) {
        if (strcmp(bank_accounts[i].iban, shmp->iban) == 0) {
            if (shmp->cmd == WITHDRAW) {
                bank_accounts[i].balance = bank_accounts[i].balance - shmp->amount;
            }
            if (shmp->cmd == DEPOSIT) {
                bank_accounts[i].balance = bank_accounts[i].balance + shmp->amount;
            }
